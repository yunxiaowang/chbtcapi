...
class chbtcApi:
    ...
    def getKline(self):
        data = 'needTickers=1&symbol=chbtcethcny&type=1min&since=%d' % (int(time.time()) * 1000)
    ...
    def buy(self, price, volume):
        try:
            ...
            obj = self.tradeCall(path, params)
            print('price: %f, volume: %f' % (price, volume))
            return obj['message']
        except Exception, ex:
            logging.error('chbtc queryAccount exception, %s' % str(ex))
            return None
    ...
    def buyHandledCny(self, price):
        if self.eth < 0.1:
            ...
            amount = cny / (price + 3.0)
            self.buy(price + 3.0, amount)
    ...
    def check(self):
        ...
        if d:
            ...
            # print 'last3: %f%%, last2: %f%%, last1: %f%%' % (self.increase3 * 100, self.increase2 * 100, self.increase1 * 100)
            # print('fallrate::::: ', fallrate)
            print('last: %f, up: %f, down: %f, cny: %f, eth: %f' % (lastPrice, upline, downline, self.cny, self.eth))
            if fallrate < -0.015:
                if self.eth > 0.1:
                    self.sellAll(lastPrice)
                    self.syncBalance()
            ...
            if (lastPrice > upline and buyPrice > upline and fallrate > 0.15):
                if self.eth < 0.1:
